...
from simplefold.model.torch.layers import ConditionEmbedder, FinalLayer
...
class FoldingDiT(nn.Module):
    ...
    def forward(self, noised_pos, t, feats, self_cond=None):
        ...
        atom_to_token = feats["atom_to_token"].to(noised_pos.dtype)  # [B, N, M]
        ...
        if self.use_length_condition:
            length = (
                feats["max_num_tokens"]
                .to(dtype=self.length_embedder[0].weight.dtype)
                .unsqueeze(-1)
            )
        ...
        mol_type = F.one_hot(mol_type, num_classes=4).to(noised_pos.dtype)  # [B, M, 4]
        res_type = feats["res_type"].to(noised_pos.dtype)  # [B, M, 33]
        pocket_feature = feats["pocket_feature"].to(noised_pos.dtype)  # [B, M, 4]
        ...
        atom_res_pos = self.aminoacid_pos_embedder(
            pos=atom_to_token_idx.unsqueeze(-1).to(noised_pos.dtype)
        )
        ...
        atom_pe_pos = torch.cat(
            [
                ref_space_uid.unsqueeze(-1).to(noised_pos.dtype),  # (B, N, 1)
                feats["ref_pos"],  # (B, N, 3)
            ],
            dim=-1,
        )  # (B, N, 4)
        token_pe_pos = torch.cat(
            [
                feats["residue_index"].unsqueeze(-1).to(noised_pos.dtype),  # (B, M, 1)
                feats["entity_id"].unsqueeze(-1).to(noised_pos.dtype),  # (B, M, 1)
                feats["asym_id"].unsqueeze(-1).to(noised_pos.dtype),  # (B, M, 1)
                feats["sym_id"].unsqueeze(-1).to(noised_pos.dtype),  # (B, M, 1)
            ],
            dim=-1,
        )  # (B, M, 4)
